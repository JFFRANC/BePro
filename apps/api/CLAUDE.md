...
| `clients` | `src/modules/clients/` | Client CRUD, contacts, positions (perfil completo + documentos por puesto vía R2 binding `FILES`, partial unique index `(tenant_id, position_id, type) WHERE is_active=true`; legacy `client_documents` archivado en sitio en migración 0010, endpoints legacy responden HTTP 410 Gone), AE assignments, per-client `form_config`. Feature 012: `clients.description` (≤2000) y `client_contacts.position` (≤120) end-to-end, con normalización empty→NULL (E-08) y diff de audit acotado al cambio. `getPrimaryAccountExecutiveName` resuelve el AE más antiguo asignado (R-04) y se expone en `IClientDetailDto.primaryAccountExecutiveName` para prefill del registro de candidato. |
| `candidates` | `src/modules/candidates/` | Registration with duplicate warning + privacy acknowledgement, 14-state FSM with role gating, R2 attachments (server-proxied upload, see ADR-002), admin reactivation (FR-038a), rejection/decline categories, retention-review compliance surface (FR-003a), append-only audit writes. Feature 012: `createCandidate` ahora construye un effective formConfig = BASE_CANDIDATE_FIELDS ∪ `formConfig.fields[]` (custom dropea colisiones con base), exige los 9 valores base en `additional_fields` (FR-012, 422 vía Zod), valida `positionId` contra `client_positions` activo del cliente (R-07, 400 `invalid_position`), y falla cerrado con `FormConfigTamperedError` → 500 `form_config_tampered` cuando un tenant perdió un base key (FR-011). |
